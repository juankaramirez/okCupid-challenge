    ...
    fs.readFile('input.json', 'utf8', function (err, contents) {
        ...
        for (a_profile of data.profiles) {
            ...
            for (b_profile of data.profiles) {
                ...
                for (b_answer of b_profile['answers']) {
                    ...
                    for (a_answer of a_profile['answers']) {
                        ...
                        if (a_questionId == b_questionId) {
                            ...
                            */
                            a_profile_base_importance_points += importance_points[a_importance]; //Maximum importance points obtainables by Profile B
                            b_profile_base_importance_points += importance_points[b_importance]; //Maximum importance points obtainables by Profile A
                            ...
                            */
                            ...
                            if (a_answer['acceptableAnswers'].indexOf(b_answer['answer']) !== -1) {
                                ...
                                */
                                if (a_answer['acceptableAnswers'].length < 4) {
                                    a_profile_importance_points += importance_points[a_importance];
                                }
                            }

                            //Profile B: Profile A answer vs Profile B acceptable answers
                            if (b_answer['acceptableAnswers'].indexOf(a_answer['answer']) !== -1) {
                                /*
                                    if a Profile states that any answer is acceptable, then the question is "irrelevant" (importance zero)                   We know this by checking the number of acceptable answers (4 possible answers). Then, 4 acceptable answers means the question is "irrelevant". Otherwise, the importance of the question will be met and has to be taken in count (accumulated).
                                */
                                if (b_answer['acceptableAnswers'].length < 4) {
                                    b_profile_importance_points += importance_points[b_importance];     
                                }
                            }
                        }
                    }
                ...
                a_match_satisfaction = a_profile_importance_points / a_profile_base_importance_points;

                //Also, we calculate the match score for Profile B vs Profile A
                b_match_satisfaction = b_profile_importance_points / b_profile_base_importance_points;
                ...
                var true_match = (Math.sqrt(a_match_satisfaction * b_match_satisfaction) - margin_error);
                ...
                ranking[ranking.length] = {'profileId': b_profile['id'], 'score': true_match};
            }
            ...
            */
            ...
            results[results.length] = {'profileId': a_profile['id'], 'matches': ranking};
        }
